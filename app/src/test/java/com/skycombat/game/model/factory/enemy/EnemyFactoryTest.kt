...
class EnemyFactoryTest {
    ...
    // 7
    @Test
    fun `enemy bullet factory type generation`() {
        val width = 100f
        val height = 100f
        val enemyGenerator = SeedGeneralEnemyFactory(10, DisplayDimension(width, height))
        val enemy= enemyGenerator.generate()
        when(enemy){
            PlaneEnemy::class -> assertEquals(enemy, PlaneEnemy::class)
            JetEnemy::class -> assertEquals(enemy, JetEnemy::class)
            SpaceShipEnemy::class -> assertEquals(enemy, SpaceShipEnemy::class)
        }
    }
}